...
1. **Hôm nay**: MIT (nếu có) với timer đếm ngược, DoD, hậu quả, Huỷ…; danh sách việc trong ngày nhóm Công việc / Cá nhân, kéo thả sắp thứ tự, ▶ tập trung cho bất kỳ việc nào (chọn 10 / 25 / 45 / 60 phút, ước lượng của việc, hoặc nhập số khác; app nhớ lần chọn cuối), thêm nhanh `Trả lời mail #w !3 ~20`, thanh tiến độ, Parking Lot ghi nhanh. **Việc con**: mỗi việc có checklist, dòng việc hiện `k/n`, chạm để mở và tick ngay tại chỗ. **Timesheet**: đánh xong một việc là hộp ghi giờ mở ra (mặc định = phút đồng hồ đã đo, hay ước lượng), ghi kèm ghi chú, bỏ qua được; ghi thêm bao nhiêu lần cũng được trong sheet Sửa việc (chọn ngày, phút, ghi chú), tổng giờ của việc = phút đo + phút ghi. Cột **Backlog** nằm ngay cạnh danh sách (bên phải trên màn rộng): việc chưa lên lịch, kể cả việc vừa trôi từ ngày trước (nhãn "trôi từ dd/mm"), dùng đúng dòng việc như trong ngày nên **tick xong được ngay tại chỗ**, có menu ⋯, và một nút đưa thẳng vào hôm nay. Đang chạy có +15 và Tạm dừng; **tắt app hay khoá máy là timer tự dừng**, mở lại thì Tiếp tục (ẩn dưới 2 phút thì tự chạy tiếp, thời gian ẩn không tính); hết giờ app hỏi "thêm 15?"; tập trung liền 25 phút thì gợi ý nghỉ 5.
2. **Kế hoạch** (một màn, ba cách nhìn, cùng một mô hình kéo thả): *Ma trận* Eisenhower cho Backlog (kéo giữa 4 ô, kéo vào "Hôm nay"/"Mai"), *Inbox* từ Parking Lot (kéo vào ô hoặc chạm Q1–Q4 để thành việc), bộ lọc khu vực, nhắc khi Backlog quá 30. *Tuần*: 7 ngày, kéo việc giữa ngày hoặc về Backlog. *Tháng*: ô ngày hiện xong/tổng, xong hết tô đậm, chấm xanh đóng ngày đúng giờ, chấm đỏ ngủ muộn; kéo việc từ Backlog hoặc từ chi tiết ngày vào một ô để đặt ngày; chạm ngày quá khứ xem việc + phút tập trung + giờ đóng + giờ ngủ (việc đã xong thì khoá, việc còn dở vẫn kéo ra / đổi lịch được); chi tiết ngày (hôm nay và quá khứ) có **Timesheet**: từng việc đã làm bao nhiêu phút (đo + ghi tay) và tổng; bên dưới thống kê tháng (lọc khu vực) và biểu đồ cột việc xong mỗi ngày.
3. **Mục tiêu**: Tuần ▸ Quý ▸ Năm, gắn mục tiêu con vào mục tiêu cha, đếm việc gắn, **check-in** đúng hướng / chậm / kẹt kèm một dòng ghi chú, nhắc khi quá hạn check-in hoặc quá 3 mục tiêu tuần. Review tuần: việc xong / tổng, ngày có ≥10′ MIT, đóng ngày đúng giờ, ngủ so mục tiêu, số lần quay lại sau ngày trống, số lần dời việc (gồm cả việc tự trôi qua đêm). Việc xong tính theo **ngày làm xong**, nên việc lấy thẳng từ Backlog cũng được đếm.
4. **Cài đặt**: giờ đóng ngày / ngủ / sáng, phút khởi động và gia hạn, ngưỡng nghỉ, khu vực mặc định, thông báo (âm, rung, hệ thống), ngôn ngữ, tài khoản & đồng bộ.

Dòng việc dùng chung cho mọi danh sách: tên một hàng riêng (không bị chip đẩy cụt trên điện thoại), hàng dưới là nhãn · `k/n` việc con · số phút · bước tiếp, các nút thao tác dồn sang phải.
...
3. **Authentication → URL Configuration**: Site URL = `https://<user>.github.io/<repo>/`, thêm URL đó vào Redirect URLs. Email provider để mặc định (magic link).
   Lưu ý: SMTP dựng sẵn của Supabase chỉ để thử, giới hạn **2 mail/giờ cho cả project** — gửi quá là 429 `email rate limit exceeded`. Dùng thật thì cắm SMTP riêng ở **Project Settings → Authentication → SMTP Settings** rồi nâng "Emails per hour" ở **Authentication → Rate Limits**. App tự chặn gửi lại trong 60 giây và nói rõ hai loại lỗi này bằng tiếng Việt.
...
6. Mở app: màn **đăng nhập** hiện đầu tiên → nhập email → mở mail trên cùng trình duyệt → bấm link. Khi đã cấu hình Supabase, app **bắt đăng nhập** mới vào được; không cấu hình thì chạy local, không tài khoản. Đăng xuất (Cài đặt) xoá dữ liệu trên máy, bản cloud giữ nguyên.
...
src/
  ...
  lib/        dates, clock (giờ giả lập), phase, period (tuần/quý/năm), quickAdd, calendar, metrics,
              ...
              db (Dexie v5 + put/patch/softDelete + outbox), actions + *.test.ts
  sync/       remote (interface + MemoryRemote), engine (push/pull/LWW), supabase (adapter), index (auth + lịch sync)
  components/ ui, dnd/ (SortableList, DragBoard), TaskRow, TaskSheet, SubtaskList, LogTimeModal, TodayBacklog, OverdueCard,
              QuickAdd, QuadrantChip, TaskForm, CancelFlow, FocusTimer, ParkingLot, AccountCard
