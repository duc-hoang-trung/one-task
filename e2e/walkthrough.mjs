...
// đóng việc chính → hộp ghi giờ, chip đầu là số phút đồng hồ đã đo
const lt1 = page.getByTestId('logtime')
await lt1.waitFor({ state: 'visible', timeout: 3000 })
await lt1.getByText(/đã đo/).waitFor({ state: 'visible' })
await shot('06e-log-time-mit')
await lt1.getByRole('button', { name: /^Ghi \d+′$/ }).click()
...
// Backlog panel nằm cạnh danh sách, lúc này trống
...
await page.getByPlaceholder(/Thêm việc/).fill('Gọi điện cho mẹ #p ~10')
await page.getByPlaceholder(/Thêm việc/).press('Enter')
await page.waitForTimeout(300)
await page.locator('li', { hasText: 'Gọi điện cho mẹ' }).getByLabel('Xong', { exact: true }).first().click()
const logtime = page.getByTestId('logtime')
await logtime.waitFor({ state: 'visible', timeout: 3000 })
await expectText('Ghi giờ cho việc này')
await logtime.getByPlaceholder('Ghi chú (tuỳ chọn)').fill('gọi lúc trưa')
await shot('11e-log-time')
...
await go('2026-09-10', '08:00')
const overdue = page.getByTestId('overdue')
await overdue.waitFor({ state: 'visible', timeout: 3000 })
await expectText('Việc chưa xong từ hôm trước')
await expectText('Làm 15 câu EC2')
await expectText('từ Thứ 4 · 09/09')
await shot('18-overdue')
await overdue.getByRole('button', { name: 'Hôm nay', exact: true }).first().click()
await page.waitForTimeout(400)
await overdue.getByRole('button', { name: 'Để trong Backlog' }).click()
await page.waitForTimeout(300)
...
const subInput = sheet.getByPlaceholder(/Thêm việc con/)
await subInput.fill('Bộ 1–15'); await subInput.press('Enter')
await subInput.fill('Bộ 16–30'); await subInput.press('Enter')
await sheet.getByTestId('time-section').getByLabel('phút').fill('25')
await sheet.getByTestId('time-section').getByPlaceholder('Ghi chú (tuỳ chọn)').fill('ôn tối qua')
await sheet.getByRole('button', { name: 'Ghi giờ' }).click()
await sheet.getByTestId('time-section').getByText('25′', { exact: true }).waitFor({ state: 'visible' })
await expectText('Tổng 25′')
await shot('21-sheet-subtasks-time')
await sheet.getByRole('button', { name: 'Lưu' }).click()
await page.waitForTimeout(300)
const row = page.locator('li', { hasText: 'Ôn 30 flashcard' }).first()
await row.getByLabel('0/2 việc con').click()
await row.getByRole('checkbox', { name: 'Bộ 1–15' }).click()   // controlled: trạng thái về qua liveQuery, nên không dùng check()
await row.getByLabel('1/2 việc con').waitFor({ state: 'visible' })
await row.getByText('25′', { exact: true }).waitFor({ state: 'visible' })
await shot('22-row-subtasks')

// 13c. Kế hoạch ▸ Tháng ▸ hôm nay: timesheet của ngày liệt kê việc và phút
await page.getByRole('button', { name: 'Kế hoạch' }).click()
await page.getByRole('button', { name: 'Tháng' }).click()
await page.getByRole('button', { name: /^10/ }).first().click()
const ts = page.getByTestId('timesheet')
await ts.waitFor({ state: 'visible' })
await ts.getByText('Ôn 30 flashcard').waitFor({ state: 'visible' })
